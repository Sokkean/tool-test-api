...
async function loadImportsMapModule(context) {
  const candidates = context.rpc.connectionMeta.backend === "static" ? [`${DEVTOOLS_MOUNT_PATH}${DEVFRAME_DOCK_IMPORTS_FILENAME}`, DEVTOOLS_DOCK_IMPORTS_VIRTUAL_ID] : [DEVTOOLS_DOCK_IMPORTS_VIRTUAL_ID];
  const errors = [];
...
function _executeSetupScript(entry, context) {
  ...
  return loadImportsMapModule(context).then((module) => {
    ...
    if (!importFn) return Promise.reject(new Error(`[VITE DEVTOOLS] No import found for id: ${id}`));
...
var _setupPromises = /* @__PURE__ */ new Map();
...
var docksContextByRpc = /* @__PURE__ */ new WeakMap();
...
async function init() {
  ...
  const context = await createDocksContext("embedded", await getDevframeRpcClient({ baseURL: DEVTOOLS_MOUNT_PATH }), useLocalStorage("vite-devtools-dock-state", {
    mode: "float",
    width: 80,
    height: 80,
    top: 0,
  ...
  const { DockEmbedded } = await import("./webcomponents-S6Z4NQQ6.js");
...
//# sourceMappingURL=nuxt___@nuxt_devtools___@vitejs_devtools_client_inject.js.map
